...
def compose_final_answer(question, sub_answers, model="gpt-4o", temperature=0.1, max_tokens=300):
    context = "\n".join([f"Q: {q}\nA: {a}" for q, a in sub_answers])
    logger.debug("[DEBUG] Context passed to LLM for final answer composition:\n" + context[:1000] + "\n")
    prompt = (
