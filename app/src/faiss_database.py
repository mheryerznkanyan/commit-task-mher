...
class FaissDatabase:
    ...
    def add_chunks(self, chunks: List[Dict]):
        ...
        """
        vectors = np.stack([np.array(chunk['embedding']) for chunk in chunks]).astype('float32')
        self.index.add(vectors)
        self.chunks.extend(chunks)
        logger.info(f"Added {len(chunks)} chunks to FAISS index")
    
    def search(self, query: str, top_k: int = 5, filter_arxiv_id: Optional[str] = None) -> List[Dict]:
        """
    ...
    def save(self, path: str = "faiss_index"):
        ...
        logger.info(f"Saved FAISS index and chunks to {path}.index and {path}.chunks.pkl")
    ...
    def load(self, path: str = "faiss_index"):
        ...
        logger.info(f"Loaded FAISS index and chunks from {path}.index and {path}.chunks.pkl")
    ...
    def add_paper(self, arxiv_id: str, title: str, summary: str, link: str, chunks: List[Dict]) -> bool:
        """
    ...
    def delete_paper(self, arxiv_id: str) -> bool:
        ...
        try:
            ...
            self.chunks = [chunk for chunk in self.chunks if chunk.get('arxiv_id') != arxiv_id]
            # Rebuild the index
            self.index = faiss.IndexFlatIP(self.embedding_dim)
            if self.chunks:
                vectors = np.stack([np.array(chunk['embedding']) for chunk in self.chunks]).astype('float32')
                self.index.add(vectors)
            return True
        except Exception as e:
