...
class SemanticChunker:
    ...
    def create_chunks(
        self, sentences: List[str], chunk_size: int = 5, overlap: int = 2
    ) -> List[Dict]:
        """
    ...
    def merge_similar_chunks(
        self, chunks: List[Dict], similarity_threshold: float = 0.85
    ) -> List[Dict]:
        """
        ...
        for i in range(len(chunks)):
            ...
            similar_indices = [
                idx for idx in similar_indices if idx not in used_indices
            ]
            ...
            if len(similar_indices) > 1:
                ...
                merged_sentences = [
                    sent for chunk in similar_chunks for sent in chunk["sentences"]
                ]
                ...
                merged_chunk = {
                    ...
                    "start_sentence_idx": min(
                        c["start_sentence_idx"] for c in similar_chunks
                    ),
                    "end_sentence_idx": max(
                        c["end_sentence_idx"] for c in similar_chunks
                    ),
