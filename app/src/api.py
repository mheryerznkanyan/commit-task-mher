...
class ComplexQARequest(BaseModel):
    question: str = Field("How are transformer models used in healthcare?", min_length=1)
...
def faiss_exists():
    ...
    """
    try:
        exists = os.path.exists(FAISS_PATH + ".index") and os.path.exists(FAISS_PATH + ".chunks.pkl")
    ...
    try:
        for ext in [".index", ".chunks.pkl"]:
            ...
            if os.path.exists(path):
                os.remove(path)
        is_empty = not (os.path.exists(FAISS_PATH + ".index") or os.path.exists(FAISS_PATH + ".chunks.pkl"))
